...
import {
  GraduationCap, Search, Save, CheckCircle2,
  Loader2, BookOpen, Users, ChevronDown, AlertCircle, Trophy, BarChart3,
  Bell, BellOff
} from 'lucide-react';
...
const TeacherGrades: React.FC = () => {
  ...
  const fetchRankings = async (groupId: number, quarter: string) => {
    ...
    try {
      const res = await api.get(`/grades/ranking/${groupId}?quarter=${quarter}`);
    ...
    if (viewMode === 'ranking') {
      ...
      try {
        ...
        groupStudents.forEach((s) => {
          const existing = existingGrades.find((g: any) => g.student?.id === s.id || g.student_id === s.id);
          map[s.id] = {
            studentId: s.id,
  ...
  const handleSave = async () => {
    ...
    setSaving(false);
    if (successCount > 0) toast.success(`${successCount} grade(s) saved successfully`);
  ...
  const toggleAnnounce = async () => {
    ...
    try {
      ...
      setExams(prev => prev.map(e => e.id === selectedExam.id ? { ...e, is_announced: newVal } : e));
      toast.success(newVal ? 'Grades announced to students! 📢' : 'Grades hidden from students.');
    } catch {
  ...
  return (
    <div className="space-y-8 pb-12">
      <div>
        <h1 className="text-3xl font-bold text-slate-800">Grade Management 🎓</h1>
        <p className="text-slate-500 mt-1">Select an exam and assign grades to your students.</p>
      </div>
          ...
          <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-4">
              ...
              {['All', 'Q1', 'Q2', 'Q3', 'Q4'].map(q => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${
                    filterQ === q ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                  }`}
                >{q}</button>
              ))}
            </div>
            ...
            <div className="space-y-2 max-h-[60vh] overflow-y-auto pr-1">
              {loading && !selectedExam ? (
                ...
                <p className="text-center text-slate-400 py-8 text-sm">No exams found</p>
                ...
                >
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <span className="font-bold text-slate-800 text-sm leading-tight">{exam.title}</span>
                    <span className={`shrink-0 px-2 py-0.5 rounded-full text-[10px] font-black ${quarterColors[exam.quarter]}`}>
                      {exam.quarter}
                    </span>
                  </div>
        ...
        <div className="lg:col-span-2">
          {!selectedExam ? (
            <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-12 text-center h-full flex flex-col items-center justify-center">
              ...
              <h3 className="text-xl font-bold text-slate-800 mb-2">Select an Exam</h3>
              <p className="text-slate-500">Choose an exam from the left panel to start assigning grades.</p>
            </div>
          ) : (
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`px-3 py-1 rounded-full text-xs font-black ${quarterColors[selectedExam.quarter]}`}>
                    ...
                    <h2 className="text-xl font-bold text-slate-800">{selectedExam.title}</h2>
                    <p className="text-sm text-slate-500 mt-0.5">
                      {selectedExam.course.title} · {selectedExam.group.name} · Max: {selectedExam.max_score || 20}/20
                    </p>
                  </div>
                  ...
                  <div className="flex gap-2 bg-slate-100 p-1.5 rounded-2xl">
                    <button 
                      ...
                      className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${
                        viewMode === 'grading' ? 'bg-white text-primary-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                      }`}
                    >Grading</button>
                    <button 
                      onClick={() => {
                        setViewMode('ranking');
                        fetchRankings(selectedExam.group.id, selectedExam.quarter);
                      }}
                      className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${
                        viewMode === 'ranking' ? 'bg-white text-primary-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                      }`}
                    >Ranking</button>
                  </div>
                  ...
                  {viewMode === 'grading' && (
                    <div className="flex items-center gap-3">
                      <button
                        ...
                        title={selectedExam.is_announced ? 'Visible to students' : 'Hidden from students'}
                      >
                        {selectedExam.is_announced ? <Bell className="w-4 h-4 fill-current" /> : <BellOff className="w-4 h-4" />}
              ...
              {viewMode === 'grading' ? (
                <>
                  {loading ? (
                    ...
                    <div className="p-12 text-center text-slate-400">
                      ...
                      <p className="font-medium">No students found in this group</p>
                    </div>
                  ) : (
                      ...
                      <table className="w-full">
                        <thead>
                          <tr className="bg-slate-50/80">
                            <th className="text-left text-xs font-black text-slate-400 uppercase tracking-wider px-6 py-4">#</th>
                            <th className="text-left text-xs font-black text-slate-400 uppercase tracking-wider px-6 py-4">Student</th>
                            <th className="text-left text-xs font-black text-slate-400 uppercase tracking-wider px-6 py-4">Grade /20</th>
                            <th className="text-left text-xs font-black text-slate-400 uppercase tracking-wider px-6 py-4">Remark</th>
                            <th className="text-left text-xs font-black text-slate-400 uppercase tracking-wider px-6 py-4">Status</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-50">
                          {filteredStudents.map((student, idx) => {
                            const entry = gradeEntries[student.id] || { grade: '', remark: '', studentId: student.id };
                            const gradeNum = parseFloat(entry.grade);
                            const isPassing = !isNaN(gradeNum) && gradeNum >= 10;
                              ...
                              <tr key={student.id} className="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td className="px-6 py-4">
                                  <input
                                    ...
                                    onChange={(e) => setGradeEntries(prev => ({
                                      ...prev,
                                      [student.id]: { ...prev[student.id], studentId: student.id, grade: e.target.value }
                                    }))}
                                ...
                                <td className="px-6 py-4">
                                  <input
                                    ...
                                    onChange={(e) => setGradeEntries(prev => ({
                                      ...prev,
                                      [student.id]: { ...prev[student.id], studentId: student.id, remark: e.target.value }
                                    }))}
                ...
                <div className="p-8">
                  {rankingLoading ? (
                    ...
                    <div className="text-center py-12 text-slate-400">No rankings available for this quarter yet.</div>
                  ) : (
                    <div className="space-y-4">
                      {rankings.map((rank, idx) => (
                        <div key={rank.student_id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <p className="font-bold text-slate-800">{rank.name}</p>
                              <p className="text-xs text-slate-500">{rank.exams_count} exams taken</p>
                            </div>
                          </div>
                          <div className="text-right">
                            <div className="text-xl font-black text-primary-600">{rank.average}</div>
