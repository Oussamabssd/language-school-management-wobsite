...
const ParentDashboard: React.FC = () => {
  ...
  const [gradeQuarter, setGradeQuarter] = useState<"Q1" | "Q2" | "Q3" | "Q4">(
    "Q1",
  );
    ...
    const fetchTabData = async () => {
      ...
      try {
        ...
        if (activeTab === "grades") {
          ...
          QUARTERS.forEach((q) => {
            normalized[q] = raw[q] ? Object.values(raw[q]) : [];
          });
          setData(normalized);
        } else {
          res = await api.get(
            `/parent/students/${selectedStudent.id}/${activeTab}`,
          );
        ...
        if (activeTab === "grades") {
          ...
          setUpcomingExams(
            allExams.filter(
              (e: any) => e.exam_date >= today && e.status === "scheduled",
            ),
          );
        } else {
          setUpcomingExams([]);
  ...
  const activeGrades =
    activeTab === "grades" ? gradeData[gradeQuarter] || [] : [];
  const calcAvg = (grades: any[]) => {
    if (!grades.length) return "—";
    return (
      grades.reduce((a, g) => a + parseFloat(g.grade), 0) / grades.length
    ).toFixed(1);
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Monitor your children's progress and attendance.
          </p>
        </div>
              ...
              <div className="space-y-4 pt-4 border-t border-slate-50">
                <div className="flex items-center gap-3 text-sm">
                  ...
                  <span className="text-slate-600 font-medium">
                    {selectedStudent.groups?.[0]?.name || "No Group"}
                  </span>
                </div>
                <div className="flex items-center gap-3 text-sm">
                  <BookOpen className="w-4 h-4 text-slate-400" />
                  <span className="text-slate-600 font-medium">
                    {selectedStudent.groups?.[0]?.level?.name || "Beginner"}
                  </span>
                </div>
              </div>
            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {(data as any[]).length > 0 ? (
                            (data as any[]).map((item: any) => (
                              <div
                                key={item.id}
                                className="p-4 rounded-2xl border border-slate-50 bg-slate-50/50 flex gap-4"
                              >
                        ...
                        <div className="space-y-4">
                          {(data as any[]).length > 0 ? (
                            (data as any[]).map((item: any) => (
                              <div
                                key={item.id}
                                className="flex items-center justify-between p-4 rounded-2xl border border-red-50 bg-red-50/30"
                              >
                                <div className="flex items-center gap-4">
                                  ...
                                  <div>
                                    <p className="font-bold text-slate-800">
                                      Absent: {item.course?.title || "Class"}
                                    </p>
                            ...
                            <div className="mb-8 space-y-3">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                {upcomingExams.map((exam) => (
                                  <div
                                    key={exam.id}
                                    className="p-4 rounded-2xl border border-primary-100 bg-primary-50/30 flex justify-between items-center"
                                  >
                                    <div>
                                    ...
                                    <div className="text-right">
                                      <span className="block text-[10px] font-black text-primary-600 bg-white px-2 py-0.5 rounded-lg border border-primary-100">
                              ...
                              <button
                                ...
                                className={`px-4 py-2 rounded-xl text-xs font-black transition-all ${
                                  gradeQuarter === q
                                    ...
                                    : `${quarterColors[q]} hover:opacity-80`
                                }`}
                              >
                                {q}
                              </button>
                            ...
                            <div className="flex items-center gap-3 mb-4 px-4 py-3 bg-slate-50 rounded-xl border border-slate-100">
                              ...
                              <span className="text-sm font-bold text-slate-700">
                                ...
                                <span className="text-primary-600">
                                  {calcAvg(activeGrades)}/20
                                </span>
                              </span>
                              <span className="text-sm text-slate-400">·</span>
                              <span className="text-sm text-slate-500">
                                {activeGrades.length} exam(s)
                              </span>
                            </div>
                          )}
                                    ...
                                    <div className="flex items-start justify-between gap-4">
                                      <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-1">
                                          ...
                                          <span
                                            className={`px-2 py-0.5 rounded-full text-[10px] font-black ${quarterColors[item.exam?.quarter]}`}
                                          >
                                            {item.exam?.quarter}
                                          </span>
                                        </div>
                                        ...
                                        <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                                          <div
                                            className={`h-full rounded-full ${isPassing ? "bg-green-400" : "bg-red-400"}`}
                                            style={{ width: `${percent}%` }}
                                          />
                                        </div>
                                        {item.remark && (
                                      ...
                                      <div className="text-right shrink-0">
                                        ...
                                        {gradeNum >= maxScore * 0.8 && (
                                          <Star className="w-4 h-4 text-yellow-400 fill-yellow-400 mt-1 ml-auto" />
                                        )}
                                      </div>
                                    </div>
                                  </div>
                          ...
                          {(data as any[]).length > 0 ? (
                            (data as any[]).map((item: any) => (
                              <div
                                ...
                                className="p-5 rounded-2xl border border-slate-50 bg-slate-50/50"
                              >
                            ...
                            <div className="py-12 text-center text-slate-400">
                              No teacher remarks available at this moment.
                            </div>
                          )}
                        </div>
                      )}
