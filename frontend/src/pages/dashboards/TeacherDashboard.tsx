...
const TeacherDashboard: React.FC = () => {
  ...
  const [selectedSession, setSelectedSession] = useState<Timetable | null>(
    null,
  );
  ...
  const fetchTeacherData = async () => {
    ...
    try {
      const timetableRes = await api.get<ApiResponse<Timetable[]>>(
        `/timetables/teacher/${user.id}`,
      );
      const timetableData =
        (timetableRes.data.data as any).data || timetableRes.data.data;
  ...
  const handleSaveAttendance = async () => {
    ...
    try {
      const today = new Date().toISOString().split("T")[0];
      const absences = Object.entries(sessionAttendance).map(
        ([studentId, data]) => ({
              ...
              {timetables.length === 0 ? (
                ...
                timetables
                  ...
                  .map((item) => (
                    <div
                      key={item.id}
                      className="p-5 hover:bg-slate-50 transition-colors group/item"
                    >
              ...
              <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    {selectedGroup.students?.length || 0} students enrolled
                  </p>
                </div>
                <button
                  onClick={() => setSelectedGroup(null)}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="p-6 border-b border-slate-100 bg-slate-50/50">
                ...
                <p className="text-sm text-slate-500">
                  {selectedSession.group?.name} • {selectedSession.day_of_week}{" "}
                  • {selectedSession.start_time.substring(0, 5)}
                </p>
              </div>

              <div className="p-6 max-h-[60vh] overflow-y-auto space-y-3">
                {!selectedSession.group?.students ||
                selectedSession.group.students.length === 0 ? (
                  <p className="text-center py-8 text-slate-400 italic font-medium">
                    No students in this group.
                  </p>
                ) : (
                  selectedSession.group.students.map((student) => {
                    const isAbsent =
                      sessionAttendance[student.id]?.status === "absent";
                    return (
                      <div
                        key={student.id}
              ...
              <div className="p-6 bg-slate-50 border-t border-slate-100 flex gap-3">
                ...
                <button
                  ...
                  disabled={
                    savingAttendance || !selectedSession.group?.students?.length
                  }
