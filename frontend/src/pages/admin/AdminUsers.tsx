...
const AdminUsers: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingUser) {
        ...
        toast.success(
          "User created successfully. A temporary password has been set.",
        );
      }
  ...
  const handleDelete = async (id: number) => {
    if (
      !confirm(
        "Are you sure you want to delete this user? This will perform a soft delete.",
      )
    )
      return;
    try {
                ...
                Array.from({ length: 5 }).map((_, i) => (
                  <tr key={i} className="animate-pulse">
                    <td className="px-6 py-4">
                      <div className="h-10 w-10 bg-slate-100 rounded-full inline-block mr-3"></div>
                      <div className="h-4 bg-slate-100 rounded w-24 inline-block align-middle"></div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap gap-1">
                        {user.roles.map((role) => (
                          <span
                            key={role.id}
                            className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded-full text-[10px] font-bold uppercase tracking-wider border border-slate-200"
                          >
                            {role.name}
                          </span>
                        ))}
              ...
              <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    {editingUser
                      ? `Updating account for ${editingUser.name}`
                      : "Fill in the details to create a new staff or parent account"}
                  </p>
                </div>
                <button
