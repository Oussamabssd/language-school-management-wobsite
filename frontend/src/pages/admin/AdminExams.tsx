...
const AdminExams: React.FC = () => {
  ...
  const toggleAnnounce = async (exam: Exam) => {
    try {
      ...
      toast.success(
        exam.is_announced ? "Exam unannounced" : "Exam announced! 📢",
      );
      fetchData();
    } catch (error) {
  ...
  const filteredExams = exams.filter((exam) => {
    const matchQuarter =
      filterQuarter === "All" || exam.quarter === filterQuarter;
    const matchGroup =
      filterGroup === "All" || exam.group.id.toString() === filterGroup;
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Schedule and manage exams for all groups and quarters.
          </p>
        </div>
        <button
          onClick={() => {
          ...
          {filteredExams.map((exam) => (
            ...
            >
              <div className="flex justify-between items-start mb-6">
                ...
                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    ...
                    className="p-2 bg-slate-50 text-slate-600 rounded-xl hover:bg-primary-50 hover:text-primary-600 transition-all"
                  >
              ...
              <div className="mt-6 flex items-center justify-between gap-2">
                ...
                >
                  <Bell
                    className={`w-3 h-3 ${exam.is_announced ? "fill-current" : ""}`}
                  />
            ...
            >
              <div className="p-8 md:p-12 overflow-y-auto max-h-[90vh]">
                ...
                <p className="text-slate-500 mb-8">
                  Fill in the details to {editingExam ? "update" : "create"} the
                  exam.
                </p>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="pt-8 flex gap-4">
                    ...
                    <button
                      type="submit"
                      disabled={submitting}
                      className="flex-1 py-4 bg-slate-900 hover:bg-slate-800 text-white rounded-2xl font-bold transition-all shadow-xl shadow-slate-200 disabled:opacity-50"
                    >
                      {submitting ? (
