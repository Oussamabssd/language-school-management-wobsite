...
import { 
  FileText, Check, X, Loader2, Download, 
  Mail, Phone, Calendar, Briefcase, UserPlus,
...
const AdminTeacherApplications: React.FC = () => {
  ...
  const handleAccept = async (id: number) => {
    if (!confirm('Are you sure you want to accept this application and create a teacher account?')) return;
    ...
    try {
      ...
      if (res.data.data.temporary_password) {
        alert(`Account created! Temporary password: ${res.data.data.temporary_password}\nPlease share this with the teacher.`);
      }
  ...
  return (
    <div className="space-y-8 pb-12">
      <div>
        <h1 className="text-3xl font-bold text-slate-800">Teacher Job Applications</h1>
        <p className="text-slate-500 mt-2">Review CVs and recruit new teachers for the school.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-3xl border border-dashed border-slate-200 p-20 text-center">
          ...
          <p className="text-slate-500">Wait for potential teachers to submit their CVs.</p>
        </div>
      ) : (
          ...
          {applications.map((app) => (
            <motion.div 
              ...
              className={`bg-white rounded-3xl border shadow-sm overflow-hidden transition-all ${
                app.status === 'accepted' ? 'border-green-100 opacity-75' : 
                app.status === 'rejected' ? 'border-red-100 opacity-75' : 'border-slate-100'
              }`}
            >
              <div className="p-8">
                <div className="flex justify-between items-start mb-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h2 className="text-xl font-bold text-slate-800">{app.first_name} {app.last_name}</h2>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-sm font-medium text-slate-600">{new Date(app.created_at).toLocaleDateString()}</p>
                  </div>
                </div>
                ...
                {app.status === 'pending' && (
                  <div className="flex gap-4">
                    ...
                    >
                      {processingId === app.id ? <Loader2 className="w-5 h-5 animate-spin" /> : <UserPlus className="w-5 h-5" />}
