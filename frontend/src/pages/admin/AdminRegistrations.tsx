...
import { 
  Check, X, Eye, Filter, Search, ChevronLeft, ChevronRight, 
  Calendar, Mail, Phone, MapPin, BookOpen, User, Clock, AlertCircle, GraduationCap
} from 'lucide-react';
...
const AdminRegistrations: React.FC = () => {
  ...
  const fetchRegistrations = async () => {
    setLoading(true);
    try {
      const response = await api.get<ApiResponse<PaginatedResponse<Registration>>>('/registrations', {
        params: {
  ...
  const handleAccept = async (id: number) => {
    if (!confirm('Are you sure you want to accept this registration? An account will be created for the student.')) return;
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'Failed to accept registration');
    }
  };
  ...
  const handleReject = async () => {
    ...
    try {
      await api.put(`/registrations/${selectedReg?.id}/reject`, { rejection_reason: rejectionReason });
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500">Manage and review student applications</p>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50/50 border-b border-slate-100">
              <tr>
                ...
                <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Language / Level</th>
                  ...
                  <tr key={reg.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="font-bold text-slate-800">{reg.full_name}</span>
                        <span className="text-xs text-slate-500">{reg.email}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm">
                      <div className="flex flex-col">
                        <span className="font-medium text-slate-700">{reg.language?.name}</span>
                        <span className="text-xs text-slate-400">{reg.level?.name || 'Level TBD'}</span>
                      </div>
                    </td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            ...
            >
              <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                ...
                <button onClick={() => setSelectedReg(null)} className="p-2 hover:bg-slate-200 rounded-full transition-all">
                  <X className="w-5 h-5" />
                </button>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-slate-800">{selectedReg.email}</p>
                    </div>
                  </div>
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-slate-800">{selectedReg.phone}</p>
                    </div>
                  </div>
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-slate-800">{new Date(selectedReg.date_of_birth).toLocaleDateString()}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-slate-800">{selectedReg.address}</p>
                    </div>
                  </div>
                  <div className="flex items-start">
                    <BookOpen className="w-5 h-5 text-primary-500 mr-3 mt-0.5" />
                    <div>
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Language Interest</p>
                      <p className="font-semibold text-slate-800">{selectedReg.language?.name}</p>
                    </div>
                  </div>
                  <div className="flex items-start">
                    <GraduationCap className="w-5 h-5 text-primary-500 mr-3 mt-0.5" />
                    <div>
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Preferred Level</p>
                      <p className="font-semibold text-slate-800">{selectedReg.level?.name || 'TBD'}</p>
                    </div>
                  </div>
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-slate-800">{new Date(selectedReg.created_at).toLocaleString()}</p>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="px-8 pb-8">
                  <div className="bg-red-50 border border-red-100 p-4 rounded-2xl flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-red-600">{selectedReg.rejection_reason}</p>
                    </div>
                  </div>
                </div>
              )}
      ...
      <AnimatePresence>
        {showRejectModal && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            ...
            >
              <h3 className="text-xl font-bold text-slate-800 mb-2 text-center">Reject Registration</h3>
              <p className="text-slate-500 text-center mb-6 text-sm">Please provide a reason why this registration is being rejected.</p>
