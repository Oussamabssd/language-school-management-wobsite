...
const StudentExamNotes: React.FC = () => {
  ...
  const totalGrades = QUARTERS.reduce(
    (acc, q) => acc + quarterData[q].length,
    0,
  );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Track your academic performance across all quarters.
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {upcomingExams.map((exam) => (
              <div
                key={exam.id}
                className="bg-white p-5 rounded-3xl border border-primary-100 shadow-sm shadow-primary-50 relative overflow-hidden group"
              >
                ...
                <div className="relative">
                  <div className="flex justify-between items-start mb-4">
                    <span
                      className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${quarterColors[exam.quarter as "Q1"].badge}`}
                    >
                      {exam.quarter}
                    </span>
                  ...
                  <div className="flex items-center justify-between text-xs font-bold pt-4 border-t border-slate-50">
                    ...
                    <span className="flex items-center gap-1 text-primary-600 bg-primary-50 px-2 py-1 rounded-lg">
                      <MapPin className="w-3.5 h-3.5" /> {exam.classroom}
                    </span>
                  </div>
                </div>
              </div>
        ...
        {QUARTERS.map((q) => {
          ...
          return (
            ...
            >
              ...
              <span
                className={`inline-block px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest mb-3 ${colors.badge}`}
              >
                {q}
              </span>
              <p className="text-2xl font-black text-slate-800">{avg || "—"}</p>
              <p className="text-xs text-slate-400 font-medium mt-0.5">
                {grades.length} exam{grades.length !== 1 ? "s" : ""}
              </p>
            </button>
          );
        })}
      ...
      <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span
              className={`px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest ${quarterColors[activeQuarter].badge}`}
            >
              {activeQuarter}
            </span>
            <h2 className="text-xl font-bold text-slate-800">
              Quarter {activeQuarter.replace("Q", "")} Results
            </h2>
          </div>
          {avgScore && (
            <div className="flex items-center gap-2 bg-slate-50 rounded-xl px-4 py-2">
              ...
              <span className="text-sm font-bold text-slate-700">
                Avg: <span className="text-primary-600">{avgScore}/20</span>
              </span>
            </div>
          )}
        </div>
            ...
            {activeGrades.length === 0 ? (
              <div className="p-16 text-center">
                ...
                <h3 className="text-lg font-bold text-slate-800 mb-1">
                  No results yet for {activeQuarter}
                </h3>
                <p className="text-slate-400">
                  Your grades will appear here once your teacher assigns them.
                </p>
              </div>
            ) : (
                    ...
                    >
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="text-right shrink-0">
                          ...
                          <span
                            className={`inline-block mt-2 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                              isPassing
