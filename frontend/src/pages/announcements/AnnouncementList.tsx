...
import { 
  Bell, Calendar, User as UserIcon, 
  Tag, AlertTriangle, Info, CheckCircle, 
...
const AnnouncementList: React.FC = () => {
  ...
  const [filter, setFilter] = useState<'all' | 'students' | 'teachers' | 'staff'>('all');
    ...
    <div className="space-y-8 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-2">Stay updated with the latest news and information.</p>
        </div>
              ...
              >
                <div className="p-6 md:p-8">
                  <div className="flex flex-wrap items-center gap-3 mb-4">
                    <span className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border ${getPriorityStyles(announcement.priority)}`}>
                    ...
                    <span className="flex items-center gap-1.5 text-xs text-slate-400 font-medium ml-auto">
                      ...
                      {announcement.published_at ? new Date(announcement.published_at).toLocaleDateString(undefined, { dateStyle: 'long' }) : 'Draft'}
                    </span>
                  </div>

                  <h2 className="text-2xl font-bold text-slate-800 mb-4">{announcement.title}</h2>
                  ...
                  <div className="mt-8 pt-6 border-t border-slate-50 flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-sm font-bold text-slate-800">{announcement.author?.first_name} {announcement.author?.last_name}</p>
                      <p className="text-xs text-slate-500 font-medium">Published by {announcement.author?.roles?.[0]?.display_name || 'Staff'}</p>
                    </div>
                  </div>
                </div>
              </motion.div>
